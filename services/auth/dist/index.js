...
const PRIVATE_KEY_PATH = process.env.JWT_PRIVATE_KEY_PATH || path_1.default.join(KEYS_DIR, 'jwt_private.pem');
...
try {
    ...
    if (!fs_1.default.existsSync(PRIVATE_KEY_PATH)) {
        fs_1.default.writeFileSync(PRIVATE_KEY_PATH, DEFAULT_PRIVATE_KEY.trim());
        console.log(`Generated default private key at ${PRIVATE_KEY_PATH}`);
    }
    if (!fs_1.default.existsSync(PUBLIC_KEY_PATH)) {
        fs_1.default.writeFileSync(PUBLIC_KEY_PATH, DEFAULT_PUBLIC_KEY.trim());
...
const privateKey = fs_1.default.existsSync(PRIVATE_KEY_PATH)
    ? fs_1.default.readFileSync(PRIVATE_KEY_PATH, 'utf8')
    : DEFAULT_PRIVATE_KEY;
...
async function emitAuditEvent(userId, tenantId, action, service, payload, status) {
    try {
        const logData = {
        ...
        await axios_1.default.post(`${AUDIT_SERVICE_URL}/events`, logData, {
            timeout: 2000,
        });
    }
    catch (error) {
...
app.post('/login', async (req, res) => {
    ...
    try {
      ...
      GROUP BY u.id
    `;
        const userRes = await db_1.pool.query(userQuery, [email]);
        ...
        if (requiresMFA) {
            ...
            const secretRes = await db_1.pool.query(secretQuery, [user.id]);
            ...
            const isMfaValid = otplib_1.authenticator.verify({
                token: mfaCode,
        ...
        const accessToken = jsonwebtoken_1.default.sign(accessTokenPayload, privateKey, { algorithm: 'RS256' });
        // Refresh token: standard 7-day duration, stored in DB for rotation & revocation
        const refreshTokenValue = jsonwebtoken_1.default.sign({ sub: user.id }, privateKey, { algorithm: 'RS256', expiresIn: '7d' });
        const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
        // Save refresh token
        await db_1.pool.query(`INSERT INTO refresh_tokens (user_id, token, expires_at) VALUES ($1, $2, $3)`, [user.id, refreshTokenValue, expiresAt]);
        // Emit event
...
app.post('/refresh', async (req, res) => {
    ...
    try {
        ...
        let decoded;
        try {
            decoded = jsonwebtoken_1.default.verify(refreshToken, publicKey, { algorithms: ['RS256'] });
        }
        catch (err) {
      ...
      WHERE r.token = $1
    `;
        const tokenRes = await db_1.pool.query(tokenQuery, [refreshToken]);
        ...
        if (tokenRecord.revoked) {
            ...
            await db_1.pool.query(`UPDATE refresh_tokens SET revoked = TRUE WHERE user_id = $1`, [userId]);
        ...
        await db_1.pool.query(`UPDATE refresh_tokens SET revoked = TRUE WHERE id = $1`, [tokenRecord.id]);
      ...
      GROUP BY u.id
    `;
        const userRes = await db_1.pool.query(userQuery, [userId]);
        const user = userRes.rows[0];
        ...
        const newAccessToken = jsonwebtoken_1.default.sign(newAccessTokenPayload, privateKey, { algorithm: 'RS256' });
        const newRefreshTokenValue = jsonwebtoken_1.default.sign({ sub: user.id }, privateKey, { algorithm: 'RS256', expiresIn: '7d' });
        const newExpiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
        // Save new refresh token
        await db_1.pool.query(`INSERT INTO refresh_tokens (user_id, token, expires_at) VALUES ($1, $2, $3)`, [user.id, newRefreshTokenValue, newExpiresAt]);
...
app.get('/users', authenticateJWT, async (req, res) => {
    ...
    try {
      ...
      `;
        ...
        const usersRes = await db_1.pool.query(query, params);
